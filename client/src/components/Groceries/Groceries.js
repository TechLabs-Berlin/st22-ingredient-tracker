...
const Groceries = () => {
    ...
    useEffect(() => {
      const search = async () => {
        const { data } = await axios.get(
          "https://api.edamam.com/auto-complete?app_id=00fa302d&app_key=8f64835541ddc5b406b34fcb4b83def9&",
          {
            params: {
              app_id: "00fa302d",
    ...
    if (groceries.length < 1) {
      return (
        <p className=" is-danger">
          empty :( please add at least one item to your inventory
        </p>
      );
    } else {
      return (
            ...
            return (
              <button
                className={`${
                  !item.selected
                    ? "button is-rounded"
                    : "button is-rounded is-warning is-light is-outlined"
                }`}
                key={itemIndex}
              ...
              >
                ...
                <button
                  ...
                  //stopPropagation() will prevent the click event to be effective on other elements other than the delete button
                  onClick={(event) => {
                    event.stopPropagation();
                    deleteItem(item);
                  }} // this works with my backend routes
      ...
      <div className="columns">
        <div className="card column is-half">
          <div className="card-content">
            <h3 className="card-header-title is-centered title is-5 has-text-primary">
              Vegetables & Greens
            </h3>
  ...
  return (
    <section id="groceries">
      ...
      <br></br>
      <br></br>
      <Link to="/recipe_detail">
        <button className="button has-text-primary" id="findRecipeButton">
          Go to Recipe Detail
        </button>
      </Link>
      <br></br>
